...
As part of our project, we also aim to detect whether or not an athlete steps out of bounds during the throw or not. To achieve this, we use [Trill Flex](https://www.mouser.com/ProductDetail/Bela/160706-FLEX?qs=Jm2GQyTW%2FbgkTnESEDaPvw%3D%3D) capacitive touch sensors that are housed within a 3D printed casing that send signals to an Arudino Uno via I2C. The Arduino then sends signals to our server running on a PC via UART which contains information as to whether or not an infraction occurred.
